...
- Before tokenizing, preserve the submitted expression verbatim as `原始输入`. Normalize each `\_` outside a quoted string to `_` only in a separate `规范化输入`, visibly state `已将字符串外的 \_ 按 _ 规范化。`, and use that copy for parsing. If no eligible `\_` occurs, `原始输入` and `规范化输入` must be byte-for-byte identical. Never append a missing token or repair the expression. `<EOF>` is an audit boundary only and must never be included in either expression.
