...
try {
    ...
    $featureSet = Join-Path $repositoryRoot 'src/test/resources/external-golden-cases/advertising-model-feature-set.json'
    $cases = Join-Path $repositoryRoot 'src/test/resources/external-golden-cases/advertising-offline-cases.json'
    & java -ea -cp $classpath com.example.featuredag.blackbox.ExternalGoldenCasesVerifier $featureSet $cases
    if ($LASTEXITCODE -ne 0) {
        throw "Advertising golden case verification failed with exit code $LASTEXITCODE"
    }
} finally {
    Pop-Location
}
